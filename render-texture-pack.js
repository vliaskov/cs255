...
function drawSprite(spritename, posX, posY) {
    for (i = 0; i < gSpriteSheets.length; i++) {
        sprite = gSpriteSheets[i].getStats(spritename);
        if ( sprite != null) {
            this.__drawSpriteInternal(sprite, gSpriteSheets[i], posX, posY);
        }
    }
...
function __drawSpriteInternal(spt, sheet, posX, posY) {
    ...
    if (spt == null)
        return;
    ctx.drawImage(sheet.img, spt.x, spt.y, spt.w, spt.h, posX + spt.cx, posY + pos.cy, spt.w, spt.h);
